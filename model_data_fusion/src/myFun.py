...
# Clapp and Hornberger, 1978, WRR 土壤属性参数
...
# (Jackson et al., 1996, Oecologia; Jackson et al., 1997, PNAS) 根形态参数
...
# constants and photosynthesis parameters 光合参数
rhow = 1e3; g = 9.81
a0 = 1.6; 
...
def LightExtinction(DOY,lat,x):
    ...
    # 用于计算大气光学厚度(atmospheric optical thickness), 用于计算大气光学厚度(atmospheric optical thickness)
    return Vegk
...
# Penman-Monteith 方程, 用于计算植被表面潜在蒸散发(ET)的物理模型.
# E 是植被表面的潜在蒸散发(以mm/day为单位);
# rnet: 是植被表面净辐射(以W/m^2为单位);
# vpd 是饱和蒸汽压差(k Pa)
# Delta 是饱和蒸汽压和温度之间的斜率(以kPa/℃为单位);
# lambdamol(λ) is the latent heat of vaporization
# Cp is the specific heat capacity of air
# ghg, gvg, ghl and gvl are the conductances 
#       for heat (superscript h) or water vapor (superscript v) 
#       from the ground surface (subscript g) or the canopy surface (subscript l) 
#       to the atmosphere above the canopy (height of eddy covariance measurement).
...
def f_PM_hydraulics(df,SRparas,theta):
    ...
    theta = [
        gpmax: gpmax = 3900 # maximum xylem conductance, 10^-9 m/s/MPa
        p50: p50 = -5.86 # psi_50, MPa
        aa: aa = 4.53 # shape parameter of vulnerability curve, unitless
        lww: lww = 6255 # marginal water use efficiency under a well-watered condition # umol/mol
        b0: b0 = -1.28 # sensitivity of MWUE to leaf water potential, /MPa
        0: 
        soil_b: soil_b = 0.62 # calibration coefficient for soil hydraulic parameter, unitless
        boundary_cond: boundary_cond = 0.253 # boundary condition of the second soil layer, in volumnetric water content
    ]
    """
...
#   gpmax,p50,aa,lww,b0
    ...
    # Vegk (k) is the canopy extinction coefficient calculated using solar zenith angle assuming a spherical leaf angle distribution
    RNg = df['RNET']*np.exp(-df['LAI']*df['Vegk']) 
    RNl = df['RNET']-RNg
    
    ggh = np.array(df['GA_U']) # ggh: conductances for heat from the ground surface
    ggv = np.array(1/(1/ggh+1/df['GSOIL']))
    
    """
    ggv: conductances for water from the ground surface
    gsoil is the soil surface obtained based on the top layer soil moisture
    """
...
# 计算ET based on gs(stomatal conductance at a leaf level); glv(冠层水力空气动力传导速率)
# SPAC model
def calSPAC(Clm,glh,SRparas,XLparas,KS,Init):
    ll = MWU(XLparas.lww,XLparas.b0,ca,Init.psil_avg)
    gs = f_carbon(Clm,ll) # mol CO2 /m2 leaf area /s 调用光合过程
